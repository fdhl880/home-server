...
function safeReplace(str, target, replacement) {
    ...
    // Using a function as the second parameter prevents $ pattern substitution!
    return str.replace(target, () => replacement);
}

// 1. Title & Account text
content = safeReplace(
    content,
...
const oldExecBtn = `                        <!-- Execute DMA Button -->
                        <button class="btn-exec" id="btnEmsxExec" onclick="executeEmsxOrder()">
                            &lt;TRANSACT / EXECUTE DMA ORDER &lt;GO&gt;&gt;
                        </button>`;

const newExecBtn = `                        <!-- Execute DMA Button & Quick Test Buy -->
...
content = safeReplace(content, oldExecBtn, newExecBtn);
...
content = safeReplace(content, oldUniverseHeader, newUniverseHeader);

// Add ACTION column to EMSX table header
...
content = safeReplace(content, oldUniverseTh, newUniverseTh);
...
content = safeReplace(content, oldMonHeader, newMonHeader);

// 6. JavaScript Logic: Add category filters, quick test buy, updated render functions
const oldJsBlock = `        function renderMonitorGrid() {
...
const newJsBlock = `        // ============================================================
        ...
        async function testBuy(sym, qty = 1) {
            showPopup(\`[TEST-BUY DMA] SUBMITTING BUY FOR '\${sym}'...\`);
            try {
                ...
                return \`
                    <tr>
                        ...
                        <td style="text-align:center; white-space:nowrap;">
                            <button class="btn-act" style="background:#003311; border:1px solid #00AA44; color:#00FF66; font-weight:bold; margin-right:3px;" onclick="testBuy('\${q.symbol}')" title="Uji beli langsung 1 lot">+BUY</button>
            ...
            ubody.innerHTML = filtered.map(q => {
                ...
                return \`
                    <tr style="cursor:pointer;" onclick="setEmsxTicker('\${q.symbol}')">
                        ...
                        <td>\${q.currency === 'USD' ? '$' + q.low.toFixed(2) : fmtIdr(q.low)}</td>
                        <td style="text-align:center;">
                            <button class="btn-act" style="background:#003311; border:1px solid #00AA44; color:#00FF66; font-weight:bold; padding:1px 6px;" onclick="event.stopPropagation(); testBuy('\${q.symbol}')" title="Uji beli langsung">+BUY</button>
                        </td>
                    </tr>
                \`;
            }).join('');
...
content = safeReplace(content, oldJsBlock, newJsBlock);

// 7. Enhance command bar to support TESTBUY or BUY <TICKER> directly
const oldCmdParser = `            } else if (raw.startsWith('BUY ') || raw.startsWith('SELL ')) {`;
const newCmdParser = `            } else if (raw.startsWith('TESTBUY ') || raw.startsWith('TBUY ')) {
                const sym = raw.split(/\\s+/)[1];
                if (sym) {
                    testBuy(sym);
                }
            } else if (raw.startsWith('BUY ') || raw.startsWith('SELL ')) {`;

content = safeReplace(content, oldCmdParser, newCmdParser);

fs.writeFileSync(filePath, content, 'utf8');
console.log('Update complete! File written safely.');
